...
public class ChatModelFactory {
    ...
    @Value("${xiaozhi.ssl.skip-verification:false}")
    private boolean skipSslVerification;
    
    @Value("${xiaozhi.ssl.connect-timeout:30}")
    private int sslConnectTimeout;
    ...
    private ChatModel newOpenAiChatModel(String endpoint, String appId, String apiKey, String apiSecret, String model, Double temperature, Double topP) {
        ...
        if (skipSslVerification) {
            logger.warn("SSL证书验证已禁用，这仅适用于开发环境！");
            return createOpenAiModelWithSkippedSSL(endpoint, appId, apiKey, apiSecret, model, temperature, topP, headers);
        } else {
            return createOpenAiModelWithSSL(endpoint, appId, apiKey, apiSecret, model, temperature, topP, headers);
        }
    }
     ...
     * 创建跳过SSL验证的OpenAI模型
     */
    private ChatModel createOpenAiModelWithSkippedSSL(String endpoint, String appId, String apiKey, String apiSecret, String model, Double temperature, Double topP, MultiValueMap<String, String> headers) {
        try {
            ...
            var openAiApi = OpenAiApi.builder()
                    ...
                    .webClientBuilder(WebClient.builder()
                            ...
                            .clientConnector(new JdkClientHttpConnector(HttpClient.newBuilder()
                                    ...
                                    .connectTimeout(Duration.ofSeconds(sslConnectTimeout))
                                    .build())))
            ...
            var chatModel = OpenAiChatModel.builder()
                    ...
                    .toolCallingManager(toolCallingManager)
                    .build();
            logger.info("使用跳过SSL验证的OpenAI模型: {}", model);
            return chatModel;
        } catch (Exception e) {
            logger.error("创建跳过SSL验证的OpenAI模型失败: {}", e.getMessage(), e);
            throw new RuntimeException("无法创建OpenAI模型", e);
        }
    }
     ...
     * 创建正常SSL验证的OpenAI模型
     */
    private ChatModel createOpenAiModelWithSSL(String endpoint, String appId, String apiKey, String apiSecret, String model, Double temperature, Double topP, MultiValueMap<String, String> headers) {
        try {
            // LM Studio不支持Http/2，所以需要强制使用HTTP/1.1
            var openAiApi = OpenAiApi.builder()
                    .apiKey(StringUtils.hasText(apiKey) ? new SimpleApiKey(apiKey) : new NoopApiKey())
            ...
            var chatModel = OpenAiChatModel.builder()
                    ...
                    .toolCallingManager(toolCallingManager)
                    .build();
            logger.info("使用正常SSL验证的OpenAI模型: {}", model);
            return chatModel;
        } catch (Exception e) {
            logger.error("创建OpenAI模型失败: {}", e.getMessage(), e);
            throw new RuntimeException("无法创建OpenAI模型", e);
        }
    }
